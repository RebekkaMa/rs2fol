...
fun Grammar<PositiveRDFSurface>.tryParseToEnd(input: String): ParseResult<PositiveRDFSurface> {
...
fun Grammar<PositiveRDFSurface>.parseToEnd(input: String): PositiveRDFSurface {
    if (input.contains("\\b_:BN_\\d+\\b".toRegex())) throw RDFSurfacesParseException("Invalid blank node Label. Please rename all blank node labels that have the form 'BN_[0-9]+'.")
...
object RDFSurfacesParser : Grammar<PositiveRDFSurface>() {
    ...
    (variableList and
        ...
        val newHayeGraph = buildList<HayesGraphElement> {
            when (surface.iri) {
                IRIConstants.LOG_POSITIVE_SURFACE_IRI -> add(PositiveRDFSurface(variableList, hayeGraph))
                IRIConstants.LOG_NEGATIVE_TRIPLE_IRI -> add(NegativeTripleRDFSurface(variableList, hayeGraph))
                IRIConstants.LOG_QUERY_SURFACE_IRI -> add(QueryRDFSurface(variableList, hayeGraph))
                IRIConstants.LOG_NEGATIVE_SURFACE_IRI -> add(NegativeRDFSurface(variableList, hayeGraph))
                IRIConstants.LOG_NEUTRAL_SURFACE_IRI -> add(NeutralRDFSurface(variableList, hayeGraph))
