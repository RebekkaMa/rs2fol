...
class RDFSurfaceParseServiceImpl : Grammar<PositiveSurface>(), RDFSurfaceParseService {
    ...
    private val numericLiteral by numericLiteralToken use { createNumericLiteral(this.text) }
    ...
    private val rdfLiteral by string and optional(langTag or ((circumflex and iri) use { t2 })) use {
        when (val part = this.t2) {
            null -> DefaultLiteral(
                ...
                datatypeIRI = from(IRIConstants.XSD_STRING_IRI)
            )

            is String -> LanguageTaggedString(
                lexicalValue = this.t1,
                langTag = part.drop(1),
            )
            ...
            is IRI -> {
                ...
                DefaultLiteral(lexicalValue = lexicalValue, datatypeIRI = part)
            }
    ...
    fun createNumericLiteral(numericLiteral: String): Literal {
        return when {
