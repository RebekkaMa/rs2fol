...
class RDFSurfaceToFOLControllerTest : ShouldSpec(
    {
        afterTest {
            RDFSurfacesParser(false).resetAll()
        }
        should("transform example2.n3 without exception") {
            ...
            (Transformer().toFOL(RDFSurfacesParser(false).parseToEnd(file.readText()))).replace("\n", " ") shouldBeEqualComparingTo solutionFile.readText().replace("\n", " ")
        }
        ...
        should("transform example10.n3 without exception") {
            ...
            println((Transformer().toFOL(RDFSurfacesParser(false).parseToEnd(file.readText()))))
            (Transformer().toFOL(RDFSurfacesParser(false).parseToEnd(file.readText()))).replace("\n", " ") shouldBeEqualComparingTo solutionFile.readText().replace("\n", " ")
        }
        ...
        context("ownExamples") {
            ...
            println(
                (Transformer().toFOL(RDFSurfacesParser(false).parseToEnd(file.readText())))
            )
        }
        ...
        context("lists"){
            should("transform blogic lists.n3") {
                ...
                Transformer().toFOL(RDFSurfacesParser(false).parseToEnd(file.readText()),true) shouldBeEqualComparingTo solutionFile.readText()
            }
        }
    })
